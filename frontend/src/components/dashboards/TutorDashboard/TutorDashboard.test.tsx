...
describe('TutorDashboard Component', () => {
  ...
  describe('Tutor Header and Welcome', () => {
    it('should render personalized welcome message', async () => {
      ...
      expect(screen.getByText(/Here's an overview of your timesheets and earnings/i)).toBeInTheDocument();
    });
  ...
  describe('Main Content Layout', () => {
    it('should display primary action buttons inside the timesheet card', async () => {
      ...
      expect(cardScope.getByRole('button', { name: /Create New/i })).toBeInTheDocument();
    });
    ...
    it('should display status at a glance', async () => {
      ...
      expect(statsScope.getByText(`${inProgressCount} In Progress`)).toBeInTheDocument();
    });
  });
  ...
  describe('My Timesheets Section', () => {
    ...
    it('should allow bulk submission of drafts', async () => {
      ...
      const draftTimesheets = mockTutorTimesheets.timesheets.filter(t => t.status === 'DRAFT' || t.status === 'MODIFICATION_REQUESTED');
      expect(screen.getByText(`${draftTimesheets.length} timesheets will be submitted`)).toBeInTheDocument();
    });
    ...
    it('should show timesheet status with clear indicators', async () => {
      ...
      mockTutorTimesheets.timesheets.forEach((timesheet) => {
        const row = within(timesheetsRegion).getAllByText(timesheet.courseName as string)[0].closest('tr');
  ...
  describe('Timesheet Form Modal', () => {
    it('should open the form modal on create', async () => {
      ...
      const timesheetsRegion = screen.getByRole('region', { name: /My Timesheets/i });
      const cardScope = within(timesheetsRegion);
      const createButton = cardScope.getByRole('button', { name: /Create New/i });
      await user.click(createButton);

      expect(await screen.findByRole('heading', { name: /New Timesheet Form/i })).toBeInTheDocument();
    });
  });
  ...
  describe('Sidebar Widgets', () => {
    it('should display comprehensive pay summary', async () => {
      ...
      const paySummaryHeading = screen.getByRole('heading', { name: /Pay Summary/i });
      ...
      expect(paySummaryScope.getByText('Average per Timesheet:')).toBeInTheDocument();
  ...
  describe('Notifications Panel', () => {
    ...
    it('should allow dismissing notifications', async () => {
      ...
      const dismissButton = within(deadlineNotification as HTMLElement).getByRole('button', { name: /Dismiss/i });
  ...
  describe('Error Handling and Edge Cases', () => {
    it('should handle empty timesheet state', async () => {
      ...
      expect(screen.getByText(/Create your first timesheet to get started/i)).toBeInTheDocument();
    });
  ...
  describe('Accessibility', () => {
    it('should have proper heading hierarchy', async () => {
      ...
      const timesheetsTitle = await screen.findByTestId('timesheets-section-title');
    ...
    it('should support keyboard navigation', async () => {
      ...
      const modalHeading = await screen.findByRole('heading', { name: /New Timesheet Form/i });
