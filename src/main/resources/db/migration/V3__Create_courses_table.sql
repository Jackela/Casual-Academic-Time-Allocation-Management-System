...
CREATE INDEX IF NOT EXISTS idx_course_code ON courses(code_value);
CREATE INDEX IF NOT EXISTS idx_course_lecturer ON courses(lecturer_id);
CREATE INDEX IF NOT EXISTS idx_course_semester ON courses(semester);
...
COMMENT ON COLUMN courses.code_value IS 'Unique course code identifier (e.g., COMP5349)';
...
COMMENT ON COLUMN courses.budget_allocated_currency IS 'Currency (ISO 4217) for allocated budget';
COMMENT ON COLUMN courses.budget_used_currency IS 'Currency (ISO 4217) for used budget';
