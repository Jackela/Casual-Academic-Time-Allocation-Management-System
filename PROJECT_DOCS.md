...
- Added elastic wait in `frontend/e2e/pages/TutorDashboardPage.ts#waitForMyTimesheetData` to resolve when any of: API response, table visible, empty state visible, error visible, or network idle, then hide loading state. This stabilizes Tutor flows without bypassing assertions.
- Switched admin status filter verification in `frontend/e2e/modules/admin-workflow.spec.ts` to content-based probe by asserting option texts via `allTextContents()` instead of visibility checks, avoiding false negatives on collapsed selects.
## Project Overview
