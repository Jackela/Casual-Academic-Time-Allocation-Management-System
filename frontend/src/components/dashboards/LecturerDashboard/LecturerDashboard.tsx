...
import { usePendingTimesheets } from "../../../hooks/timesheets";
import {
  useApprovalAction,
...
const StatCard = memo<StatCardProps>(
  ({ title, value, subtitle, trend = "stable", icon }) => (
...
const LecturerDashboard = memo<LecturerDashboardProps>(({ className = "" }) => {
  ...
  } = useTimesheetDashboardSummary({ scope: "lecturer" }); // Lecturer dashboard, not admin
  ...
  const handleRejectionSubmit = useCallback(
    async (reason: string) => {
      ...
      try {
        ...
        await Promise.all([refetchPending(), refetchDashboard()]);
        setSelectedTimesheets((prev) =>
          prev.filter((id) => id !== rejectionModal.timesheetId),
        );
      } catch (error) {
  ...
  return (
    ...
    >
      <header className="mb-8 flex items-center justify-between">
        <div>
          <h1
            ...
            data-testid="main-welcome-message"
          >
            {welcomeMessage}
          </h1>
          <p
            className="text-muted-foreground"
            data-testid="main-dashboard-title"
          >
            Lecturer Dashboard
          </p>
          {urgentCount > 0 && (
            ...
            >
              ...
              <span data-testid="urgent-count">{urgentCount}</span> urgent
              approvals needed
            </div>
          )}
        </div>
      ...
      <section className="mb-8" role="region" aria-label="Dashboard Summary">
        <div
