...
test.describe('Admin Dashboard Workflow', () => {
  ...
  test('Admin can filter timesheet table by status', async ({ page }) => {
    ...
    await expect(statusFilter.locator('option:has-text("All Statuses")')).toBeVisible();
    await expect(statusFilter.locator('option:has-text("Draft")')).toBeVisible();
    await expect(statusFilter.locator('option:has-text("Pending")')).toBeVisible();
    await expect(statusFilter.locator('option:has-text("Approved")')).toBeVisible();
    await expect(statusFilter.locator('option:has-text("Rejected")')).toBeVisible();
  ...
  test('Admin can use override approve action on pending timesheets', async ({ page }) => {
    ...
    // Check if timesheets table exists and has pending timesheets
    const timesheetTable = page.locator('[data-testid="timesheets-table"]');
    const tableVisible = await timesheetTable.isVisible().catch(() => false);
  ...
  test('Admin dashboard shows appropriate actions for different timesheet statuses', async ({ page }) => {
    ...
    if (tableVisible) {
      ...
      // If no timesheets, should show empty state
      await expect(page.locator('[data-testid="empty-state"]')).toBeVisible();
      await expect(page.locator('text=No timesheets')).toBeVisible();
    }
  });
});